    ...
    try {
      ...
      const tab = await new Promise((resolve) =>
        chrome.tabs.query({}, (tabs) =>
          resolve(
            tabs.find(
              ({ active, windowId }) => active && windowId === currentWindow.id
            )
          )
        )
      );
      ...
      await new Promise((resolve) => {
        setTimeout(resolve, backoffRate ** attempt * baseTimeout);
      });
    }
  }
...
export async function run(/** @type {Array} */ steps) {
  ...
  while (queue.length > 0) {
    ...
    try {
      ...
      await new Promise((resolve) => {
        setTimeout(resolve, backoffRate ** currentAttempt * baseTimeout);
      });
      queue.unshift(step);
    }
  }
