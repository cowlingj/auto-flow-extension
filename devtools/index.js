...
import parseExtensions from "../utils/parse-extensions.js";
...
chrome.devtools.panels.create(
  ...
  async function (panel) {
    ...
    const loadedCurrentFlowName = await new Promise((resolve) =>
      chrome.storage.local.get(["currentFlowName"], ({ currentFlowName }) =>
        resolve(currentFlowName)
      )
    );
    ...
    panel.onShown.addListener((/** @type {Window}*/ win) => {
      ...
      function refreshStepThrough() {
        win.document.querySelector("#step-through-input").value = currentFlow
          .steps[currentStep]
          ? JSON.stringify(currentFlow.steps[currentStep], null, 2)
          : "";
      }

      function refreshList() {
        const availableFlowList = win.document.querySelector(
          "#available-flow-list"
        );
        availableFlowList.textContent = "";
        const availableFlowTemplate = win.document.querySelector(
          "#available-flow-item"
        ).content;
        Object.entries(flows).forEach(([key, value]) => {
          ...
          const downloadAnchor = clone.querySelector(
            "#available-flow-item-download"
          );
          downloadAnchor.href = `data:application/json;base64,${btoa(
      ...
      function addFlow() {
        ...
        reader.readAsText(
          win.document.querySelector("#add-flow-file").files[0]
        );
      }
      ...
      function setStepThroughChanged(changed) {
        if (changed) {
          const downloadCurrentAnchor = win.document.querySelector(
            "#step-through-download"
          );
          downloadCurrentAnchor.href = `data:application/json;base64,${btoa(
      ...
      win.document
        .querySelector("#step-through")
        .addEventListener("click", () => toggle("#step-through-controls"));

      win.document.querySelector("#add-flow").addEventListener("click", () => {
        hide("#remove-flow-controls");
        show("#add-flow-controls");
      });
      win.document
        .querySelector("#remove-flow")
      ...
      win.document
        .querySelector("#add-flow-name")
        .addEventListener("change", () => {
          win.document.querySelector("#add-flow-confirm").disabled = false;
        });
      win.document
        .querySelector("#add-flow-confirm")
        .addEventListener("click", () => {
          const newFlowName = win.document.querySelector("#add-flow-name")
            .value;
      ...
      win.document
        .querySelector("#remove-flow-confirm")
        .addEventListener("click", () => {
      ...
      win.document
        .querySelector("#add-flow-overwrite-confirm")
        .addEventListener("click", () => {
          try {
            addFlow();
            win.document.querySelector("#add-flow-name").value = "";
      ...
      win.document
        .querySelector("#add-flow-overwrite-cancel")
        .addEventListener("click", () => {
          hide("#add-flow-overwrite-controls");
        });

      win.document
        .querySelector("#add-flow-cancel")
        .addEventListener("click", () => hide("#add-flow-controls"));
      win.document
        .querySelector("#remove-flow-cancel")
      ...
      win.document.querySelector("#run").addEventListener("click", async () => {
        try {
          ...
          await run(parseExtensions(currentFlow.steps, currentFlow.extensions));
          currentStep = 0;
        } catch (e) {
          content.error(serializeError(e));
        } finally {
      ...
      win.document
        .querySelector("#step-through-run")
        .addEventListener("click", async () => {
          const steps = makeArray(
            JSON.parse(win.document.querySelector("#step-through-input").value)
          );
          const parsedSteps = parseExtensions(steps, currentFlow.extensions);
          const changed =
            JSON.stringify(steps) !==
            JSON.stringify([currentFlow.steps[currentStep]]);
          try {
            setDisabledAll(".disable-on-run", true);
            validate({ ...currentFlow, steps: parsedSteps });
            await run(parsedSteps);
      ...
      win.document
        .querySelector("#step-through-skip")
        .addEventListener("click", () => {
          currentStep += 1;
      ...
      win.document
        .querySelector("#step-through-back")
        .addEventListener("click", () => {
          currentStep -= 1;
          if (currentStep < 0) {
      ...
      win.document
        .querySelector("#step-through-insert-before")
        .addEventListener("click", () => {
      ...
      win.document
        .querySelector("#step-through-insert-after")
        .addEventListener("click", () => {
          currentStep += 1;
          currentFlow.steps.splice(currentStep, 0, null);
          refreshStepThrough();
      ...
      win.document
        .querySelector("#step-through-delete")
        .addEventListener("click", () => {
      ...
      win.document
        .querySelector("#step-through-overwrite")
        .addEventListener("click", () => {
